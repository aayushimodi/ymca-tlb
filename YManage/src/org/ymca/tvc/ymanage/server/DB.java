...
public class DB {
	...
	synchronized AttendanceRecord checkInVolunteer(String name) {
		...
		if (!volunteers.containsKey(name)) {
			throw new YException(
					"The volunteer " + name + " does not exist. The board must add them to the volunteer list.");
		}
	...
	synchronized ArrayList<MeetingId> getPastMeetingIds() {
		ArrayList<MeetingId> result = new ArrayList<>(meetings.keySet());
		Collections.sort(result);
		return result;
	}
	...
	private void loadCurrentMeeting() {
		...
		if (currentMeetingFile.exists()) {
			...
			try {
				this.currentMeeting = gson.fromJson(new FileReader(currentMeetingFile.getAbsolutePath()),
						MeetingAttendanceStatus.class);
	...
	private void loadAllMeetings() {
		...
		for (File f : meetingFiles) {
			if (!f.getName().startsWith("current")) {
				...
				try {
					MeetingAttendanceStatus m = gson.fromJson(new FileReader(f.getAbsolutePath()),
							MeetingAttendanceStatus.class);
	 ...
	 * Checkin Student
	 * 
	 * Is there a meeting going on? No - return error
	 * 
	 * Is there a student with the given name No - return error
	 * 
	 * Is the student already checked in in the current meeting?
	 * 
	 * Yes - return error already checked in
	 * 
	 * Add to current meeting the attendance record for this student Add to the
	 * student the attendance record
	 * 
	...
	/*
	 * Start a meeting Is there already meeting in progress Yes return error
	 * 
	 * Create new current meeting Add record false to all students for this meeting
	 * 
	 * 
	 */

	/*
	 * Stop a meeting Is there a meeting in progress No return error
	 * 
	 * Take the current meeting and add it to the history Reset the current meeting
	 * to null
	 */
